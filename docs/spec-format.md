...
When the `--dev` CLI flag is used (or implicitly activated for curation workflows), `nbw` will apply these overrides. When the `--prod` flag is used, these overrides are explicitly ignored. When `--finalize-dev-overrides` is used, this section is deactivated.
