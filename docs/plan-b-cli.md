...
Use normal mamba, pip, or uv installs and deletes then `net save`.
...
Creates an archive file (`can`) of the specified environment defaulting to
the first pantry in `NBW_PATH` or the pantry specified by `NBW_SAVE`
...
Default is current environment.
...
#### List available environments (`ls [e-glob]`)
